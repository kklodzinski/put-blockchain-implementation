...
{  
    ...
    blockchain.add_transaction(0, 1, 20);
    blockchain.add_transaction(0, 2, 20);
    blockchain.add_transaction(0, 3, 20);
    blockchain.add_transaction(0, 4, 20);
    blockchain.add_transaction(0, 5, 20);
    blockchain.add_transaction(0, 6, 20);
    blockchain.add_transaction(0, 7, 20);
    blockchain.add_transaction(0, 8, 20);
    blockchain.add_transaction(0, 9, 20);
    blockchain.add_transaction(0, 10, 20);
    ...
    // Init Server
    int opt = TRUE;  
    int master_socket , addrlen , new_socket , client_socket[30], miner_socket[30],
          max_clients = 30 , activity, i , valread , sd, sd_m;  
    int max_sd;  
    struct sockaddr_in address;
    unsigned int miner_id_for_socket[30];
    ...
    while(TRUE)  
    {
        ...
        {  
            ...
            std::cout << "New connection is: " << identity << std::endl;
            ...
            if (std::strcmp(identity, "client") == 0) {
            ...
            } else if (std::strcmp(identity, "minerr") == 0) {
                //Sync miner

                // Receive public key, to send rewards correctly
                recv(new_socket, key_buffer, sizeof(key_buffer), 0);
                std::cout << "buffer: \n" << key_buffer << std::endl;
                BIO_puts(bio, key_buffer);
                public_key = PEM_read_bio_RSAPublicKey(bio, &public_key, nullptr, nullptr);
                ...
                new_miner = user_list.at(std::string(key_buffer));
                ...
                blockchain.get_transaction_block_hash(hash);
                send(new_socket, hash, SHA256_DIGEST_LENGTH, 0);
                ...
                //add new socket to array of miner sockets
                for (i = 0; i < max_clients; i++) {  
                    //if position is empty
                    if(miner_socket[i] == 0) {
                        miner_socket[i] = new_socket;
                        miner_id_for_socket[i] = new_miner;
                        printf("Adding to list of sockets as %d\n" , i);
        ...
        // Calculate block to use if no miners are available and send ledger to miners
        new_block = false;
        if (ledger.size() >= 10) {
            std::cout << "Generating new block" << std::endl;
            blockchain.get_transaction_block_hash(hash);
            for (i = 0; i < 10; i++) {
                current_transaction = ledger.front();
                blockchain.add_transaction(current_transaction);
                ledger.erase(ledger.begin(), ledger.begin() + 1);
            }
            current_block = blockchain.create_transaction_block(hash);
            //ledger.erase(ledger.begin(), ledger.begin() + 10);
            new_block = true;
        }

        // New block has priority
        for (i = 0; i < max_clients; i++) {
            sd = miner_socket[i];
        ...
        {  
            ...
            if (FD_ISSET( sd , &readfds))  
            {
                ...
                if (valread == sizeof(put::blockchain::block_chain::transaction_t)) {
                    ledger.push_back(current_transaction);
                    std::cout << "added transaction " << ledger.size() << std::endl;

                    // Send new transaction to miners
                    for (int j = 0; j < max_clients; j++) {
                        sd_m = miner_socket[j];
                        if (sd_m > 0) {
                            std::cout << "Sending transaction to miner" << std::endl;
                            send(sd_m, &current_transaction, sizeof(put::blockchain::block_chain::transaction_t), 0);
                        }
                    }
                }
                ...
                if (new_block) {
                    send(sd, "yes", 4, 0);
                    send(sd, &current_block, sizeof(put::blockchain::block_chain::transaction_block_t), 0);
                } else {
                    send(sd, "noo", 4, 0);
                }
            }  
